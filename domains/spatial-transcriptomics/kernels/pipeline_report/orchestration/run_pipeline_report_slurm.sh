#!/bin/bash
#
# TumorSPACE AWS Pipeline - Pipeline Report SLURM Submission (Step 12)
# Generates pipeline_summary.md and Pipeline_Report.pdf for a completed run
# Runs automatically as Step 12 after BayesSpace (Step 11) completes
#
...
# Handle case where directory doesn't exist yet (will be created by Step 10)
if [[ "$OUTPUT_DIR" = /* ]]; then
    # Already absolute
    :
else
...
# This prevents them from being deleted when Step 6 recreates optimal/
...
echo "================================================================================"
echo "TumorSPACE Pipeline Report (Step 12)"
echo "================================================================================"
...
# Step 10 (completion) will create it before Step 11 runs
...
# Validate OUTPUT_DIR exists (Step 10 should have created it)
if [ ! -d "$OUTPUT_DIR" ]; then
    echo "ERROR: Output directory does not exist: $OUTPUT_DIR"
    echo "Step 10 (completion) may have failed to create optimal/ directory"
    exit 1
fi

# Validate required input files exist
REQUIRED_FILES=("OptimalTree_preprune.nw" "OptimalTree_pruned.nw" "gene/DA_significant.txt" "OptimalTree_pruned_node_compare_list.txt" "gene/SLAB_scores.txt")
