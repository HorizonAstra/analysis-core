...
if [ "$USE_CONTAINER" = true ]; then
  ...
  # Deduplicate paths to avoid "destination already in mount point list" warnings
  # when the same directory appears more than once (e.g. joint mode reusing a dataset).
  _BIND_ARGS=()
  declare -A _BOUND_PATHS
  _save_IFS="$IFS"
  IFS=','
  # shellcheck disable=SC2206
  _PATHS=($DATAPATH)
  ...
  unset _save_IFS
  for _p in "${_PATHS[@]}"; do
    if [ -z "${_BOUND_PATHS[$_p]+x}" ]; then
      _BIND_ARGS+=(--bind "$_p":"$_p")
      _BOUND_PATHS[$_p]=1
    fi
  done
  if [ -z "${_BOUND_PATHS[$METAPATH_DIR]+x}" ]; then
    _BIND_ARGS+=(--bind "$METAPATH_DIR":"$METAPATH_DIR")
  fi
  if [ -z "${_BOUND_PATHS[$OUTDIR]+x}" ]; then
    _BIND_ARGS+=(--bind "$OUTDIR":"$OUTDIR")
  fi
  unset _p _PATHS _BOUND_PATHS
