...
singularity exec --no-home --cleanenv --bind __OUTPUT_BASE__:__OUTPUT_BASE__ __CONTAINER__ \
    sh -c 'export LD_LIBRARY_PATH=/usr/local/julia/lib/julia:$LD_LIBRARY_PATH && \
           gotree \
           -i "'"$SVD_DIR"'/Tree_0.nw" \
           -b "'"$BOOT_FILE"'" \
           -o "'"$SUPPORT_FILE"'" \
...
sed -i "s|__HPC_LOAD_SINGULARITY__|${HPC_LOAD_SINGULARITY}|g" "$SBATCH_SCRIPT"
