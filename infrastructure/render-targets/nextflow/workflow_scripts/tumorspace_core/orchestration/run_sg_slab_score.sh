...
#
# TumorSPACE Step 8: SLAB (Spatially Localized Abundance Bias) Scoring
#
...
#   - DA_significant.txt                             From Step 7
#   - OptimalTree_pruned_node_compare_list.txt       From Step 6
#   - OptimalTree_pruned_node_compare_leaves.txt     From Step 6
#
# Inputs (in data-dir/):
...
# Check required Step 7 and Step 6 outputs exist (skip if using dependency)
if [ -z "$DEPENDENCY" ]; then
    DA_FILE="$OPTIMAL_DIR/DA_significant.txt"
    if [ ! -f "$DA_FILE" ]; then
        echo "ERROR: DA_significant.txt not found in $OPTIMAL_DIR"
        echo "Make sure you have run differential abundance analysis (Step 7) first"
        exit 1
    fi
    ...
    if [ ! -f "$SG_LIST" ]; then
        ...
        echo "Make sure you have run SG identification (Step 6) first"
        exit 1
    fi
...
if [ "$TEST_MODE" = true ]; then
    rm -f "$OPTIMAL_DIR/SLAB_scores.txt"
    rm -f "$OPTIMAL_DIR/SLAB_barcodes.txt"
...
echo "=========================================="
echo "TumorSPACE Step 8: SLAB Scoring"
echo "=========================================="
...
# Copy barcodes_positions.txt to optimal directory (required by Julia script)
echo "Copying barcodes_positions.txt to optimal directory..."
cp "$BARCODE_POS" "$OPTIMAL_DIR/"
...
echo "=========================================="
echo "Submitting Step 8: SLAB Scoring"
echo "=========================================="
