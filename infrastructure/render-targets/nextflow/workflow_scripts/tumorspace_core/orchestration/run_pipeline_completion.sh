...
echo "================================================================================"
echo "TumorSPACE Pipeline - Completion Marker (Step 9)"
echo "================================================================================"
...
if [ -z "$DEPENDENCY" ]; then
    if [ ! -d "$OPTIMAL_DIR" ]; then
        ...
        echo "Have you run Step 5 (svd_selection)?"
        exit 1
    fi
fi
...
# Critical files from Step 8 that must be fully written
CRITICAL_FILES=(
    "${OPTIMAL_DIR}/SLAB_scores.txt"
    "${OPTIMAL_DIR}/SLAB_barcodes.txt"
)
...
if [ -f "${OPTIMAL_DIR}/DA_significant.txt" ]; then
    DA_GENES=$(tail -n +2 "${OPTIMAL_DIR}/DA_significant.txt" | wc -l)
...
if [ -f "${OPTIMAL_DIR}/SLAB_scores.txt" ]; then
    ...
    SLAB_SIZE=$(stat -c%s "${OPTIMAL_DIR}/SLAB_scores.txt" 2>/dev/null || echo "0")
    if [ "$SLAB_SIZE" -gt 100 ]; then
        SLAB_ENTRIES=$(tail -n +2 "${OPTIMAL_DIR}/SLAB_scores.txt" | wc -l)
