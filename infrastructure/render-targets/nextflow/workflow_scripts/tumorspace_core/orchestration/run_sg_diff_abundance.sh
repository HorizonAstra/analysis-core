...
#
# TumorSPACE Step 7: Differential Gene Expression Analysis
#
...
# Check required Step 6 outputs exist (skip if using dependency)
if [ -z "$DEPENDENCY" ]; then
...
if [ "$TEST_MODE" = true ]; then
    rm -f "$OPTIMAL_DIR/DA_results.txt"
    rm -f "$OPTIMAL_DIR/DA_significant.txt"
...
cp "$DATA_DIR/M.txt" "$OPTIMAL_DIR/"
cp "$DATA_DIR/barcodes.txt" "$OPTIMAL_DIR/"
cp "$DATA_DIR/genes.txt" "$OPTIMAL_DIR/" || cp "$DATA_DIR/features.txt" "$OPTIMAL_DIR/genes.txt"
...
echo "=========================================="
echo "Submitting Step 7: Differential Abundance"
echo "=========================================="
