...
// params_label This is a variable that captures both the stat_q and the metaphlan_db values to label the parameters that were used in the analysis for anything downstream of metaphlan
def params_label = params.stat_q == '0.2' ? "${params.metaphlan_db}_statq_${params.stat_q}" : "${params.metaphlan_db}_statq_${params.stat_q}"
println "Debug: params_label = ${params_label}"
// params.baqlava_params_label this is a version of the params label that has special characters removed 
def baqlava_params_label = params_label.replaceAll("[^a-zA-Z0-9]", "_")
// This is for adding the stat_q variable at the end of some strings if the value isn't the default 
def check_statq = params.stat_q == '0.2' ? "_statq_${params.stat_q}" : "_statq_${params.stat_q}"
println "Debug: check_statq = ${check_statq}"
...
process clinical_rename {
    ...
    memory = 24.GB
    cpus = 6
    container = "docker://dfiuchicago/shotgun:2.7"
...
process Kneaddata_new { 
    ...
    --trimmomatic /usr/local/lib/Trimmomatic-0.39/ \
    --trimmomatic-options="ILLUMINACLIP:${params.trimmomatic_location}/adapters/TruSeq3-PE-2.fa:${params.ILLUMINACLIP_params}" \
...
process Metaphlan { 
    ...
    output: 
    tuple val(sample_id), path(reads), file(hash_metadata), file(knead_cat_reads), file("taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt"), val(database), file(kneaddata_log), path(fastqc_folder), path(root_dir), emit: meta_tuple
    path ("taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt"), emit: meta_tax_profile
    tuple val(sample_id), path("taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt"), path("taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt"), emit: meta_tax_profile_bowtie
    ...
    if [ -f \$grandparent_dir/taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome_gtdb.txt ]; then
        mkdir -p taxonomy/metaphlan/${params_label}
        echo "Prior analysis exists in target folder, skipping analysis."
        rsync -hpvir --no-times --chmod=Dg=rwx,Fg=rwx \$grandparent_dir/taxonomy/metaphlan/${params_label}/${sample_id}*_profiled_metagenome.txt taxonomy/metaphlan/${params_label}/
    else
        mkdir -p taxonomy/metaphlan/${params_label}
        metaphlan ${knead_cat_reads} --input_type fastq \
        -t rel_ab_w_read_stats \
        --profile_vsc \
        -s taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}.sam.bz2 \
        --vsc_out taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}.vsc.txt \
        --mapout taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}.bowtie2.bz2 \
        --index ${database} \
        --db_dir $metaphlandb_folder/${database}/ \
        --stat_q ${params.stat_q} \
        --nproc ${task.cpus} -o taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt

        if [ ! -s \$grandparent_dir/taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome_gtdb.txt ]; then
            echo "Converting Metaphlan species to GTDB"
            ${params.sgb2gtdb} -i taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt -o taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome_gtdb.txt -d ${params.metaphlan_db}
        fi

        samfile="taxonomy/metaphlan/${params_label}/${sample_id}.sam"
        vsc_out="taxonomy/metaphlan/${params_label}/${sample_id}.vsc.txt"
        bowtie2out="taxonomy/metaphlan/${params_label}/${sample_id}.bowtie2.bz2"
...
process Combine_metaphlan {
    ...
    """
        ...
        mkdir -p ${params.out}/${params_label}/
...
process HUMAnN { 
    ...
    output:
    path ("humann/${sample_id}/${params_label}/*.tsv")    

    script: 
    """
    grandparent_dir=\$(cat ${root_dir})
    if [ -f \$grandparent_dir/humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm_GO.tsv  ]; then
        echo "Prior analysis exists in target folder, skipping analysis."
        mkdir -p humann/${sample_id}/${params_label}
        rsync -hvir --chmod=Dg=rwx,Fg=rwx \$grandparent_dir/humann/${sample_id}/${params_label}/* humann/${sample_id}/${params_label}/
    else
        ...
        humann \
            ...
            --output-basename ${sample_id}_${params_label} \
            --remove-temp-output \
            --output humann/${sample_id}/${params_label} --threads ${task.cpus} \
            --resume \
            -v
        ...
        humann_renorm_table \
            --input humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies.tsv \
            --output humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm.tsv \
            --units cpm \
            --update-snames
        ...
        humann_renorm_table \
            --input humann/${sample_id}/${params_label}/${sample_id}_${params_label}_pathabundance.tsv \
            --output humann/${sample_id}/${params_label}/${sample_id}_${params_label}_pathabundance-cpm.tsv \
            --units cpm \
            --update-snames

        echo "Converting gene families to KEGG Orthologs"
        humann_regroup_table \
            --input humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm.tsv \
            -c /gpfs/data/dfi-cores/biobakery/databases/humann/utility_mapping/utility_mapping/map_ko_uniref90.txt.gz \
            --output humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm_KO.tsv 
        ...
        humann_regroup_table \
            ...
            --output humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm_level4EC.tsv 
        ...
        humann_regroup_table \
            ...
            --output humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm_GO.tsv 
...
process BAQLaVa { 
    ...
    output:
    path ("taxonomy/baqlava/${sample_id}/${params_label}/*_BAQLaVa_profile.txt")    
    ...
    mkdir -p taxonomy/baqlava/\${flattened_name}/${baqlava_params_label} || true

    if [ -f "\$grandparent_dir/taxonomy/baqlava/${sample_id}/${params_label}/*_BAQLaVa_profile.txt" ]; then
        echo "Prior analysis exists in target folder, skipping analysis."
        pigz -9 \$grandparent_dir/taxonomy/baqlava/${sample_id}/*/*.fa || true

        mkdir -p taxonomy/baqlava/${sample_id}/${params_label} || true
        rsync -hvir --no-times --chmod=Dg=rwx,Fg=rwx \$grandparent_dir/taxonomy/baqlava/${sample_id}/${params_label}/* taxonomy/baqlava/${sample_id}/${params_label}/ || true
        mv \$knead_cat_reads $knead_cat_reads 
    else

        baqlava -i \$knead_cat_reads -o taxonomy/baqlava/\${flattened_name}/${baqlava_params_label} \
        ...
        mv taxonomy/baqlava/${sample_id}/${baqlava_params_label} taxonomy/baqlava/${sample_id}/${params_label} || true
...
process bakta_wgs { 
    ...
    container = "docker://oschwengers/bakta:v1.11.0"
...
process gutsmash_wgs {
    ...
    errorStrategy = { task.attempt <= 8 ? 'retry' : 'ignore' }
    container = "docker://nmendozam/gutsmash:latest"
    publishDir "${params.out}", mode: 'copy', overwrite: true
...
process antismash_wgs {
    ...
    memory = { 8.GB + 8.GB * task.attempt }
    errorStrategy = { task.attempt <= 8 ? 'retry' : 'ignore' }
    container = "docker://dfiuchicago/antismash:1.0.0"
    publishDir "${params.out}", mode: 'copy', overwrite: true
    ...
    if (params.dehost_input == true) {
        ...
        if (params.biobakery_only == true) {
            ...
            root_dir_ch = read_pairs_ch.map { tuple ->
                path = tuple[1][1].toString()
                basefolder = path.tokenize('/')[0..-3].join('/')
                return basefolder
            }
        ...
        if (params.clinical_rename == true) { // this is a new feature that renames the sampleid and fastqs to fit the clinical sample id naming convention
            ...
            read_pairs_ch = read_pairs_ch.map { tuple ->
                ...
                if (firstElement.contains("_CLIN_SSG_") || firstElement.contains("_CLIN_DSG_")) {
                    ...
                    lastElementIndex = firstElement.lastIndexOf('_S')
                    ...
                    newpath = '/' + basefolder + "/" + "renamed"
                    println "new path: ${newpath}"

                    newSampleIdTuple = [newSampleId] + tuple[1..-1] + [newpath]
...
workflow Sylph_workflow {
    Channel
        ...
        .set{read_pairs_ch}
    read_pairs_ch.view()
    sylph_ch = Slyph(read_pairs_ch.collect(), reference_ch)
}
...
process Metaphlan_deHost { 
    ...
    output: 
    tuple val("${sample_id}"), path(reads), file("taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt"), path("root_dir"), emit: meta_tuple
    path ("taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt"), emit: meta_tax_profile
    ...
    if [ -f \$greatgrandparent_dir/taxonomy/metaphlan/${params_label}/${sample_id}.sam.bz2 ]; then
        mkdir -p taxonomy/metaphlan/${params_label}
        echo "Prior analysis exists in target folder, skipping analysis."
        rsync -hpvir --no-times --chmod=Dg=rwx,Fg=rwx \$greatgrandparent_dir/taxonomy/metaphlan/${params_label}/${sample_id}*_profiled_metagenome.txt taxonomy/metaphlan/${params_label}/
    else
        echo "Starting analysis. No prior analysis not found in target folder."
        mkdir -p taxonomy/metaphlan/${params_label}
        ...
        if [ ! -s \$greatgrandparent_dir/taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome_gtdb.txt ]; then
            echo "Converting Metaphlan species to GTDB"
            ${params.sgb2gtdb} -i taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome.txt -o taxonomy/metaphlan/${params_label}/${sample_id}_${params_label}_profiled_metagenome_gtdb.txt -d ${params.metaphlan_db}
        fi
...
process BAQLaVa_dehost { 
    ...
    if [ -f "\$grandparent_dir/taxonomy/baqlava/${sample_id}/${baqlava_params_label}/\${flattened_name}_${baqlava_params_label}_BAQLaVa_profile.txt" ] || [ -f "\$grandparent_dir/taxonomy/baqlava/${sample_id}/${baqlava_params_label}/\${flattened_name}_${baqlava_params_label}_BAQLaVa_profile.txt" ]; then
        echo "Prior analysis exists in target folder, skipping analysis."
        pigz -9 taxonomy/baqlava/${sample_id}/*/*.fa || true

        mkdir -p taxonomy/baqlava/${sample_id}/${baqlava_params_label} || true
        ...
        echo "Starting analysis using existing ${baqlava_params_label}. No prior baqlava analysis found in target folder."
        if [ "${params.metaphlan_db}" == "mpa_vJan25_CHOCOPhlAnSGB_202503" ] && [ -f \$grandparent_dir/taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}_profiled_metagenome.txt ]; then
            mkdir -p taxonomy/baqlava/${sample_id}/${baqlava_params_label} || true
            echo "Using tax profile from ${params_label} instead of ${params.metaphlan_db} with no temp dir"
            alternative_tax_profile=\$grandparent_dir/taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}_profiled_metagenome.txt 
            mv "\$flattened_name.fastq" "\${flattened_name}_${baqlava_params_label}.fastq" || true

            baqlava -i "\${flattened_name}_${baqlava_params_label}.fastq" -o taxonomy/baqlava/\${flattened_name}/${baqlava_params_label} --taxonomic-profile \$alternative_tax_profile --threads ${task.cpus} \
            --nucdb /gpfs/data/dfi-cores/biobakery/databases/baqlava/data/BAQLaVa.V0.5.nucleotide \
            --protdb /gpfs/data/dfi-cores/biobakery/databases/baqlava/data/BAQLaVa.V0.5.protein
            rm \${flattened_name}_${baqlava_params_label}.fastq 
            mv taxonomy/baqlava/\${flattened_name} taxonomy/baqlava/${sample_id} || true
            mv taxonomy/baqlava/${sample_id}/${baqlava_params_label} taxonomy/baqlava/${sample_id}/${params_label} || true

            pigz -9 taxonomy/baqlava/${sample_id}/${baqlava_params_label}/*.fa
            rm -f taxonomy/baqlava/${sample_id}/${baqlava_params_label}/*.fa
            mv *fa.gz taxonomy/baqlava/${sample_id}/${baqlava_params_label}/ || true
            rsync -hvir --chmod=Dg=rwx,Fg=rwx taxonomy \$grandparent_dir
        elif [ "${params.metaphlan_db}" == "mpa_vJan25_CHOCOPhlAnSGB_202503" ]; then
            echo "Running metaphlan with: Using ${params_label} because alternative profile from ${params.metaphlan_db} not found!"
            mkdir -p taxonomy/baqlava/${sample_id}/${params_label} || true
            ...
            -t rel_ab_w_read_stats \
            --profile_vsc \
            -s taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}.sam.bz2 \
            --vsc_out taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}.vsc.txt \
            --mapout taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}.bowtie2.bz2 \
            --index mpa_vJun23_CHOCOPhlAnSGB_202403 \
            --db_dir  ${params.metaphlandb_folder}/mpa_vJun23_CHOCOPhlAnSGB_202403/ \
            --stat_q ${params.stat_q} \
            --nproc ${task.cpus} -o taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}_profiled_metagenome.txt
            if [ ! -s \$greatgrandparent_dir/taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}_profiled_metagenome_gtdb.txt ]; then
                echo "Converting Metaphlan species to GTDB"
                ${params.sgb2gtdb} -i taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}_profiled_metagenome.txt -o taxonomy/metaphlan/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}_profiled_metagenome_gtdb.txt -d ${params.metaphlan_db}
            fi

            samfile="taxonomy/metaphlan/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}.sam"
            bamfile="\${samfile%.sam}.bam"
            # remove any existing bam or bai files to avoid conflicts
            ...
            baqlava -i "\${flattened_name}_mpa_vJun23_CHOCOPhlAnSGB_202403.fastq" -o taxonomy/baqlava/\${flattened_name}/mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq.replaceAll("[^a-zA-Z0-9]", "_")} \
            --taxonomic-profile taxonomy/metaphlan/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}/${sample_id}_mpa_vJun23_CHOCOPhlAnSGB_202403${check_statq}_profiled_metagenome.txt \
            --threads ${task.cpus} \
            --nucdb /gpfs/data/dfi-cores/biobakery/databases/baqlava/data/BAQLaVa.V0.5.nucleotide \
            --protdb /gpfs/data/dfi-cores/biobakery/databases/baqlava/data/BAQLaVa.V0.5.protein
            ...
            mkdir -p taxonomy/baqlava/\${flattened_name}/${baqlava_params_label} || true
            echo "Using standard tax profile for ${baqlava_params_label}"
            mv "\$flattened_name.fastq" "\${flattened_name}_${baqlava_params_label}.fastq"

            baqlava -i "\${flattened_name}_${baqlava_params_label}.fastq" -o taxonomy/baqlava/\${flattened_name}/${baqlava_params_label} --taxonomic-profile $meta_tax_profile --threads ${task.cpus} \
            --nucdb /gpfs/data/dfi-cores/biobakery/databases/baqlava/data/BAQLaVa.V0.5.nucleotide \
            --protdb /gpfs/data/dfi-cores/biobakery/databases/baqlava/data/BAQLaVa.V0.5.protein
            rm \${flattened_name}_${baqlava_params_label}.fastq
            mv taxonomy/baqlava/\${flattened_name} taxonomy/baqlava/${sample_id} || true
...
process HUMAnN_deHost { // humann reccomends using the cat reads
    ...
    pwd
    grandparent_dir=\$(cat ${root_dir})
    if [ -f \$grandparent_dir/humann/${sample_id}/${params_label}/${sample_id}_${params_label}_pathabundance-cpm.tsv ]; then
        echo "Prior analysis exists in target folder, skipping analysis."
        mkdir -p humann/${sample_id}/${params_label}
        rsync -hvir --chmod=Dg=rwx,Fg=rwx \$grandparent_dir/humann/${sample_id}/${params_label}/* humann/${sample_id}/${params_label}/
    else
        ...
        --output-basename ${sample_id}_${params_label} \
        --remove-temp-output \
        --output humann/${sample_id}/${params_label} \
        --resume \
        -v

        echo "renormalizing the table."
        humann_renorm_table --input humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies.tsv \
            --output humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm.tsv \
            --units cpm \
            --update-snames
        ...
        humann_regroup_table \
            --input humann/${sample_id}/${params_label}/${sample_id}_${params_label}_genefamilies-cpm.tsv \
            -c /gpfs/data/dfi-cores/biobakery/databases/humann/utility_mapping/utility_mapping/map_go_uniref90.txt.gz \
            --output humann/${sample_id}/${params_label}/${sample_id}_${params_label}-cpm_GO.tsv 
